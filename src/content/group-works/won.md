---
title: { en: "Won", ko: "원", ja: "ウォン" }
tagline:
  en: "The chain of our wishes and achievements completes the stone tower."
  ko: "우리들의 소원과 성취의 사슬이 돌탑을 완성한다."
  ja: "私たちの願いと成就の鎖が石塔を完成させる。"
releases:
  - { "ArtSpace@SNU 2021": "2021-10-27" }
type: Group work
genre: ["Video game", "Art game", "Public art"]
tags: ["Interactive CG", "Artificial life", "Automata"]
tools: ["Unity"]
source: https://github.com/random-entity/o.art.won/tree/after-exhibition-end
---
::: en
![](images/works/won/exh/IMG_9390.jpeg)

## Basic information

- **Format**
  - Genre: Public video game, Art game
  - Composition: A video game installed in a public space where anyone can participate; operated with a PC, monitor, keyboard, and mouse.
- **Creators**
  - Exhibitor: Hojeong Lee
  - Direction, Graphic Design: Hojeong Lee
  - Programming: random-entity, Minki Cho
- **Release**
  - Exhibition: [2021 Seoul National University Art Week: ArtSpace@SNU 2021](https://art.snu.ac.kr/notice/2021-%EC%98%88%EC%88%A0%EC%A3%BC%EA%B0%84-art-spacesnu-%ED%96%89%EC%82%AC-%EC%95%8C%EB%A6%BC/)
- **Source code**: [GitHub](https://github.com/random-entity/o.art.won/tree/after-exhibition-end)

## Synopsis

- A PC (including a monitor, keyboard, and mouse) is installed in a plaza, allowing passersby to use the keyboard to enter a nickname and the mouse to select an emoji for their current wish and a wish they have already achieved. Once the selection is complete, a stone with the current wish attached to its head, the nickname in the center, and the achieved wish attached to its tail floats in the sky.
- Among the increasing number of stones as people participate, if the head emoji of one stone matches the tail emoji of another, they are attracted to each other and stick together. As this repeats, the stones form a chain.
- In this way, when the very first head and the very last tail of a chain match, the chain sinks to the ground and a stone tower is completed.

![](images/works/won/exh/IMG_9388.jpeg)
*Nickname and wish input screen*

![](images/works/won/demo-still/21_won_1.jpg)
*Floating stones. They gradually become bound into chains of wishes and achievements.*

![](images/works/won/demo-still/21_won_2.jpg)
*Zoomed out screen*

![](images/works/won/demo-still/21_won_3.jpg)
*When the chain of wishes and achievements forms a cycle, it sinks and becomes a stone tower.*

## Commentary

- We live our lives hoping for and achieving big and small things. To a person wishing for something, someone who has already achieved it can be an object of envy or a teacher who shares their experiences. This connection between people, mediated by wishes, is the force that keeps us living and unites the community.
- Artist Hojeong Lee's thoughts are projected into a virtual world of stones that float like living organisms and gather like a community, and "stone towers" that are commonly built in our culture while making a wish.

[ArtSpace@SNU — artwork introduction](https://youtu.be/zFeyxBnuzAI)

## Technology

- The game engine Unity was used to process user input via the keyboard/mouse/GUI and to implement the algorithm for the floating and gathering of stones.

## External links

- [YouTube — ArtSpace@SNU — artwork introduction](https://youtu.be/zFeyxBnuzAI)
- [Instagram — ArtSpace@SNU — artwork introduction](https://www.instagram.com/tv/CV4R3ENl8mW/)
- [Facebook — ArtSpace@SNU](https://www.facebook.com/snuartspace)
:::
::: ko
![](images/works/won/exh/IMG_9390.jpeg)

## 기본 정보

- **형식**
  - 장르: 공공 비디오게임, 아트 게임
  - 구성: 공공장소에 설치되어 누구나 참가할 수 있는 비디오게임; PC, 모니터, 키보드, 마우스로 작동.
- **만든 사람들**
  - 출품자: 이호정
  - 연출, 그래픽 디자인: 이호정
  - 프로그래밍: 임의존재, 조민기
- **공개**
  - 전시: [2021년 서울대학교 예술주간: ArtSpace@SNU 2021](https://art.snu.ac.kr/notice/2021-%EC%98%88%EC%88%A0%EC%A3%BC%EA%B0%84-art-spacesnu-%ED%96%89%EC%82%AC-%EC%95%8C%EB%A6%BC/)
- **소스 코드**: [GitHub](https://github.com/random-entity/o.art.won/tree/after-exhibition-end)

## 줄거리

- 광장에 한 대의 PC(모니터, 키보드, 마우스 포함)가 설치되어, 지나가는 사람들이 키보드를 사용해 닉네임을, 마우스를 사용해 현재의 소원 그리고 이미 이룬 소원을 이모지로 선택할 수 있다. 선택을 완료하면 현재의 소원이 머리에, 닉네임이 중앙에, 이룬 소원이 꼬리에 붙은 돌이 하늘을 떠다니게 된다.
- 사람들의 참가에 따라 점점 늘어가는 돌들 중에서, 어느 돌의 머리와 다른 돌의 꼬리 이모지가 일치하면 그것들은 서로 끌어당겨져 붙게 되고, 이것이 반복되면서 돌들은 사슬을 형성한다.
- 그렇게 해서 어떤 사슬의 맨 앞 머리와 맨 끝 꼬리가 일치하면, 그 사슬은 지면으로 가라앉아 돌탑이 완성된다.

![](images/works/won/exh/IMG_9388.jpeg)
*닉네임 및 소원 입력 화면*

![](images/works/won/demo-still/21_won_1.jpg)
*날아다니는 돌맹이들. 점차 소원과 성취의 사슬로 묶인다.*

![](images/works/won/demo-still/21_won_2.jpg)
*줌 아웃 한 화면*

![](images/works/won/demo-still/21_won_3.jpg)
*소원과 성취의 사슬이 사이클을 이루면 가라앉아 돌탑이 된다.*

## 해설

- 우리는 크고 작은 것들을 희망하고 또 이루면서 살아간다. 어떤 일을 희망하는 사람에게, 그 일을 이미 이룬 사람은 부러움의 대상이 될 수도 있고, 경험담을 들려주는 스승이 될 수도 있다. 소망을 매개로 한 이러한 사람과 사람 간의 연결은 우리를 살아가게 하고 공동체를 결집하는 힘이다.
- 이호정 작가의 이러한 생각은 생명체처럼 부유하고 공동체처럼 결집하는 돌멩이들과, 우리 문화에서 흔히 소원을 빌면서 쌓는 '돌탑'들의 가상 세계에 투영되어 있다.

[ArtSpace@SNU — 작품 소개](https://youtu.be/zFeyxBnuzAI)

## 기술

- 키보드/마우스/GUI를 통한 유저 인풋을 처리하고, 돌멩이들의 부유와 결집 알고리즘 등을 구현하기 위해 게임 엔진 Unity를 사용했다.

## 외부 링크

- [YouTube — ArtSpace@SNU — 작품 소개](https://youtu.be/zFeyxBnuzAI)
- [Instagram — ArtSpace@SNU — 작품 소개](https://www.instagram.com/tv/CV4R3ENl8mW/)
- [Facebook — ArtSpace@SNU](https://www.facebook.com/snuartspace)
:::
::: ja
![](images/works/won/exh/IMG_9390.jpeg)

## 基本情報

- **形式**
  - ジャンル: 公共ビデオゲーム、アートゲーム
  - 構成: 公共の場に設置され、誰でも参加できるビデオゲーム。PC、モニター、キーボード、マウスで動作。
- **制作陣**
  - 出品者: LEE Hojeong
  - 演出、グラフィックデザイン: LEE Hojeong
  - プログラミング: 任意存在、CHO Minki
- **公開**
  - 展示: [2021年ソウル大学芸術週間：ArtSpace@SNU 2021](https://art.snu.ac.kr/notice/2021-%EC%98%88%EC%88%A0%EC%A3%BC%EA%B0%84-art-spacesnu-%ED%96%89%EC%82%AC-%EC%95%8C%EB%A6%BC/)
- **ソースコード**: [GitHub](https://github.com/random-entity/o.art.won/tree/after-exhibition-end)

## あらすじ

- 広場に1台のPC（モニター、キーボード、マウス含む）が設置され、通りがかりの人々がキーボードを使ってニックネームを、マウスを使って現在の願いとすでに叶えた願いを絵文字で選択できる。選択を完了すると、現在の願いが頭に、ニックネームが中央に、叶えた願いが尻尾に付いた石が空を漂うことになる。
- 人々の参加によって徐々に増えていく石の中で、ある石の頭と別の石の尻尾の絵文字が一致すると、それらは互いに引き寄せられてくっつき、これが繰り返されることで石は鎖を形成する。
- そうしてある鎖の先頭と最後尾が一致すると、その鎖は地面に沈み、石塔が完成する。

![](images/works/won/exh/IMG_9388.jpeg)
*ニックネームおよび願いの入力画面*

![](images/works/won/demo-still/21_won_1.jpg)
*飛び回る石たち。次第に願いと成就の鎖で結ばれていく。*

![](images/works/won/demo-still/21_won_2.jpg)
*ズームアウトした画面*

![](images/works/won/demo-still/21_won_3.jpg)
*願いと成就の鎖がサイクルを形成すると、沈んで石塔になる。*

## 解説

- 私たちは大小さまざまなことを願い、そして叶えながら生きている。あることを願う人にとって、それをすでに叶えた人は羨望の対象になることもあれば、経験談を聞かせてくれる師匠になることもある。願いを媒介としたこのような人と人とのつながりは、私たちを生かし、共同体を結集させる力である。
- LEE Hojeong作家のこのような考えは、生命体のように浮遊し、共同体のように結集する石たちと、私たちの文化においてよく願いを込めながら積まれる「石塔」の仮想世界に投影されている。

[ArtSpace@SNU — 作品紹介](https://youtu.be/zFeyxBnuzAI)

## 技術

- キーボード/マウス/GUIを通じたユーザー入力の処理や、石たちの浮遊と結集のアルゴリズムなどを実装するために、ゲームエンジンUnityを使用した。

## 外部リンク

- [YouTube — ArtSpace@SNU — 作品紹介](https://youtu.be/zFeyxBnuzAI)
- [Instagram — ArtSpace@SNU — 作品紹介](https://www.instagram.com/tv/CV4R3ENl8mW/)
- [Facebook — ArtSpace@SNU](https://www.facebook.com/snuartspace)
:::
