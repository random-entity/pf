...
const ROOT = join(fileURLToPath(new URL('.', import.meta.url)), '..', 'src', 'content', 'artworks')
const FM = /^---\r?\n([\s\S]*?)\r?\n---(\r?\n[\s\S]*)?$/
...
if (!key || oldValue == null || newValue == null) {
  console.error('Usage: node scripts/rename-value.mjs <key> <oldValue> <newValue> [--dry]')
  process.exit(1)
}
...
const quoteIfNeeded = (s) => (/[,:#[\]{}&*!|>'"%@`]|^\s|\s$/.test(s) ? JSON.stringify(s) : s)
...
function rewriteFrontmatter(fm) {
  ...
  if (flow.test(fm)) {
    fm = fm.replace(flow, (_m, pre, inner, post) => {
      const items = inner.split(',').map((s) => unquote(s.trim())).filter(Boolean)
      const next = []
      const seen = new Set()
  ...
  const keyLine = lines.findIndex((l) => new RegExp(`^\\s*${escapeRe(key)}:\\s*$`).test(l))
  if (keyLine !== -1) {
  ...
  // Scalar:  key: value
  const scalar = new RegExp(`^(\\s*${escapeRe(key)}:\\s*)(["']?)${escapeRe(oldValue)}\\2\\s*$`, 'm')
  if (scalar.test(fm)) {
...
for (const file of walk(ROOT)) {
  ...
  if (!m) continue
  let data
  try { data = yaml.load(m[1]) || {} } catch { continue }

  const carries = carriesValue(data)
  if (!carries) continue
  if (carries === 'localized') { localizedHits.push(file); continue }
