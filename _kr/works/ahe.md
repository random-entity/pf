...
<dl>
  ...
  <dd>
    <dl>
      <dt>출품자</dt>
      <dd>Robot Theater Company<span markdown="1">[^2]</span> (권병준<span markdown="1">[^3]</span>, 이유진, 이주미, 임의존재)</dd>
      <dt>총괄 디렉터,<br />하드웨어 설계,<br />음악</dt>
      <dd>권병준</dd>
      <dt>조감독,<br />로봇 안무</dt>
      <dd>이유진</dd>
...
<dl>
  <dt>씬 1</dt>
  <dd>
    이족보행 로봇 “아해”<span markdown="1">[^1]</span> 13대가 오와 열을 맞춰 무대에 서 있다. 골반 양끝에서부터 하늘을 향해 높이 솟은 깃대 사이에는 날개처럼 팔랑거리는 기다란 술이 매달려 있다. 아해들은 대열을 유지한 채 제자리에서 발을 앞으로 뻗었다가 뒤로 무르거나 좌우로 발목을 꺾으면서, 전자석이 바닥에 탈부착하는 소리를 구호 삼아 기계적인 군무를 추기 시작한다. 모든 아해가 제자리에서 전자석을 빠른 속도로 바닥에 탈부착시키는 장면에서의 따다닥거리는 소리와 아해들의 요동은 무대를 감싼 긴장감을 증폭시킨다.
  </dd>
  <dt>씬 2</dt>
  <dd>
...
- 아해의 기계공학적 구조:
  ...
  - 관절부는 골반과 양쪽 다리의 연결부 두 곳에 위치한다. 각 관절부는 1-DOF 서보모터 ([mjbots mj5208]) 및 컨트롤러([mjbots moteus]), 기어, 그리고 하우징 케이스로 구성된다.
...
- **센서 시스템**:
  - 특정 위치로 걸어가기 등, 아해가 무대 평면 상의 스스로의 위치와 향하고 있는 방향을 인식해야만 가능한 동작을 위해, 커스텀 LPS (local positioning system) 시스템과 IMU 센서([이투박스 EBIMU-9DOF])를 사용했다.
    - LPS 시스템은 아해에 임베드 된 LPS 모듈 기판([DW1000])으로부터 무대 상의 3개의 고정된 지점(앵커)에 설치된 LPS 모듈 기판([DW1000])까지의 거리를 측정해 삼변측량법을 통해 아해의 위치를 계산한다.
    - IMU 센서는 아해가 향하고 있는 방향을 측정한다.
  - 아해 관절부의 서보모터를 기어 출력 각위치/각속도 기준으로 제어하기 위해 기어 축에 장착된 자석과 외부 인코더를 사용했다.
  - 아해가 벽 혹은 타 개체와의 충돌에 의해 더 이상 진행할 수 없는 교착 상태에 빠졌을 경우 이를 감지해 벗어나게 하기 위해 [mjbots moteus SDK]가 제공하는 서보모터 위치 오차 값을 사용했다.
- **무선 통신 시스템**:
  - 각 아해가 걸어가는 동안 타 개체와의 충돌 회피를 위해서는 서로가 서로의 위치를 알고 있어야 하고, 또한 무대 위의 아해들을 객석 뒤에서 원격으로 제어/모니터 하기 위해서는 모든 아해들과 커맨더/모니터 시스템을 노드로 하는 무선 통신 메쉬 네트워크가 필요했다. 이를 위해 [Digi XBee]를 사용했다.
...
- **커맨더/모니터 시스템**:
  - 공연 시 로봇들에게 커맨드를 보내기 위해 [XBee] 디바이스를 USB로 연결한 PC에서 구동되는 [Pure Data] 패치를 사용했다. GUI를 통한 실시간 제어를 가능케 하기 때문에 [Pure Data]를 채택했다.
  ...
  - 공연 시 어두운 조명 속 멀리 떨어진 무대 위의 아해들의 위치/방향 및 기계적 오류 상태를 시각적으로 모니터링 하기 위해 [XBee] 디바이스를 USB로 연결한 PC에서 구동되는 [Processing] 스케치를 사용했다.
- 아해의 임베디드 시스템 (하드웨어):
  - 아해 구동에 필요한 상기 모듈들 (서보모터 컨트롤러, 전자석, 배터리, LPS, IMU, XBee 등) 간의 정보 흐름을 종합적으로 관리하기 위해 [PJRC Teensy] 마이크로컨트롤러를 사용했다. 그리고 이상의 전자 부품들의 입출력 핀 연결을 탈부착식으로 하기 위해 커스텀 PCB를 만들어 사용했다.
- **아해의 임베디드 시스템 (소프트웨어)**:
  - [Teensy SDK]와 [mjbots moteus SDK]를 사용한 C++ 코드를 아해 임베디드 시스템의 [Teensy]에 업로드함으로써, 아해가 센서 시스템 및 무선 통신 시스템과 정보를 주고받음과 동시에 공연 진행을 위한 모든 동작을 수행할 수 있게 했다.
  - 동시다발적으로 일어나는 데이터 플로우와, 시간의 흐름에 따라 전개되고 끝나는 동작을 [Teensy]의 싱글 스레드에서 처리하기 위해, 메인 루프 속에 각자의 주기로 실행되는 서브루틴들을 투입해, 멀티 스레드처럼 작용하는 싱글 스레드 프로그램을 작성했다.
- 안무:
...
[이투박스 EBIMU-9DOF]: https://www.e2box.co.kr/entry/AHRS-EBIMU9DOFV5?category=784546
